...
LLMs are trained using the language modeling objective—predicting the next word in a sequence. This pretraining task is auto-regressive and self-supervised.
...
3. **Long-Distance Dependencies**
   Use the “soups...were delicious” example to explain why N-gram models can’t capture subject–verb agreement across distant words. Connect this to the parameter explosion problem.
...
5. **Language Models in Speech Recognition**
   Describe how LMs evaluate possible transcriptions by assigning sentence-level probabilities and selecting the most probable one (e.g., disambiguating “I will be Bish”).
...
| **N-gram Model**                      | Predicts a word based on the previous n−1 words (Markov assumption).                  |
...
| **Underflow**                         | Error from multiplying small probabilities—values become too small to represent.      |
...
* **Long-Distance Dependencies**
  ...
  * N-grams struggle to model relationships between distant words (e.g., subject–verb agreement across clauses).
